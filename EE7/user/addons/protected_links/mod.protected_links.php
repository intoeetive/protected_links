...
class Protected_links {
    ...
    function _serve_file($link_id, $storage, $container, $endpoint, $url, $filename,  $type, $file_id, $inline = false)
    {
        ...
        if (strpos((string) $url, 'file:') !== false) {
            if (preg_match_all('/{file\:(\d+)\:url}/', (string) $url, $matches, PREG_SET_ORDER)) {
                $file_ids = [];
                ...
                foreach ($files as $file) {
                    if ($storage == 'local') {
                        $url = str_replace('{file:' . $file->file_id . ':url}', $file->getAbsolutePath(), $url);
                    } else {
                        $url = str_replace('{file:' . $file->file_id . ':url}', $file->getAbsoluteURL(), $url);
                    }
                }
            }
        }
        ...
        switch ($storage)
        {
            ...
            case 'url':
                if (strpos($url, '/') === 0) {
                    ...
                    // and we need full URL, so try to fix it by adding the base URL
                    $url = ee()->config->item('base_url') . trim($url, '/');
                }
                $use_curl = true;
                $size = -1;
